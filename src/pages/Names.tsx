import { creatureData } from '../data/creatureNames.json'

// For each series array, map each name to a list item
const all = creatureData.map((eachSeries) => (
  <div
    key={eachSeries.seriesId}
    className="border-t border-gray-200 pt-6 mt-6 first:border-t-0 first:mt-0 first:pt-0"
  >
    <h2 className="text-lg font-semibold mb-3 text-gray-700">
      {eachSeries.seriesTitle}
    </h2>
    <ol className="list-none space-y-0.5 text-gray-600">
      {eachSeries.names.map((creature) => (
